...
Use this workflow whenever an algorithm diagram contains a mathematical formula that ordinary PowerPoint text cannot reproduce reliably. Compile the formula with LaTeX, convert the tightly cropped PDF to SVG, inspect it through a same-aspect PNG, use that PNG as the pptfast layout asset, and replace it with the SVG during the recorded Presentation finalization pass.
...
- `inkscape`: rasterize the SVG for visual inspection and for the same-aspect pptfast preview asset; it may also be used for SVG cleanup when necessary.
- `pptfast`: place the PNG preview through deterministic IR, preview, audit, and base-PPTX rendering.
- `@oai/artifact-tool` or the available Presentations workflow: replace the accepted preview object with the SVG bytes while preserving its geometry.
- Presentation render and overflow helpers: verify the final slide rather than trusting the source SVG alone.

Prefer the pipeline

```text
formula.tex -> pdflatex -> formula.pdf -> formula.svg -> same-aspect preview PNG -> pptfast base deck -> recorded SVG replacement
```
...
The preview PNG is both a QA artifact and the layout-compatible asset for the pinned pptfast release. Keep the SVG as the final source; the PNG and SVG must share the same visible bounds and aspect ratio.

## 5. Use the PNG in pptfast, then replace it with the SVG

Register the preview PNG in pptfast `assets.images` under a stable semantic asset ID. Complete the IR validation, audit, preview, visual comparison, and base render before replacing it. Record the source SVG and preview PNG relationship in the task asset manifest.

After the pptfast preview is accepted, load the Presentations skill, work on a copy of the base deck, and replace the preview image with the SVG while preserving the object's x/y/w/h, rotation, z-order, crop behavior, name, and alt text. Use `@oai/artifact-tool` when it is the active presentation authoring backend:
...
- Preserve the geometry already accepted in the pptfast preview; do not resize it by eye during replacement.
...
- Keep the source `.tex`, preview PNG, and replacement record so the equation and final deck can be regenerated deterministically.
...
- A high-resolution same-aspect preview has been visually inspected and used in the pptfast IR.
- The SVG contains only the formula and has sensible outer padding.
- The accepted preview object is replaced by the SVG as a separate PowerPoint image object with the same geometry and alt text.
