...
6. **Generate deterministic pptfast IR from the draft.** Read [references/pptfast-generation-and-review.md](references/pptfast-generation-and-review.md) and the retained upstream operating guide [references/pptfast-upstream-skill.md](references/pptfast-upstream-skill.md). Use the standard pptfast IR directly; do not create a separate algorithm-diagram IR or reduce the draft to a structure-only text description first. Infer the target composition visually from the draft, register every scientific subfigure as an image asset, fix a seed, and pin the layout when the chosen arrangement must remain stable. Run the bundled version-pinned launcher for schema discovery, validation, asset briefs, audit, preview, and render. Whenever mathematical notation is required, also read [references/latex-formula-in-pptx.md](references/latex-formula-in-pptx.md); preserve the editable TeX source and use the derived formula asset rather than approximating notation with ordinary text.
7. **Evaluate the pptfast preview and iterate at the correct layer.** Compare the draft and preview region by region. If the problem is component choice, ordering, hierarchy, density, layout, or fit, edit the IR and rerun pptfast. If the problem is the subfigure itself, edit the asset while keeping the IR stable and rerun pptfast. Continue until both `validate` and `audit` pass and the preview preserves every retained scientific subfigure, formula, legend, branch, input, and output without a lower-information substitution.
8. **Finalize only unsupported details, then inspect the actual PPTX.** When vector-source replacement or another PowerPoint-specific correction cannot be expressed through pptfast, read [references/ppt-assembly-and-qa.md](references/ppt-assembly-and-qa.md), load the available **Presentations** skill, and apply the correction to a copy of the accepted pptfast base deck. Preserve the placeholder object's geometry when replacing an SVG preview raster, record every finalization so it can be replayed after an IR revision, render the actual final PPTX, compare it to the draft again by region, and run the presentation overflow/structure test. Broad layout changes must return to the IR loop rather than accumulating manual edits.

Only when the user explicitly asks to check, update, or synchronize the bundled pptfast version, read [references/pptfast-update-sync.md](references/pptfast-update-sync.md). Do not load that reference during ordinary diagram generation.
...
- All titles, module names, annotations, main-flow arrows, panel frames, and genuinely simple diagrams must remain native editable PowerPoint elements produced by pptfast or the finalization workflow; this rule does not apply to geometry-sensitive scientific subfigures.
- Complex subfigures must be separate embedded image objects with descriptive names and alt text; the PPTX must not depend on external paths. Treat PNG and SVG as one semantic image-resource class even when the pinned pptfast release needs a same-aspect PNG preview for an SVG source.
